# Create custom mod of ggforestplot
forestplot_mod <- function (df, name = name, estimate = estimate, se = se, pvalue = NULL, 
                            ...
                            ci = 0.95, ...) 
{
  ...
  g <- g + theme_forest() + scale_colour_ng_d(reverse = TRUE) + scale_fill_ng_d(reverse = TRUE) + 
  ...
  g <- g + geom_effect(ggplot2::aes(xmin = .data$.xmin, xmax = .data$.xmax, 
                       ...
                       position = ggstance::position_dodgev(height = 0.5), size = 1.2) + 
    ggplot2::scale_shape_manual(values = c(21L, 22L, 23L, 
                                           24L, 25L, 22L)) + guides(colour = guide_legend(reverse = TRUE), 
